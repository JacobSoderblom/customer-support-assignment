...
export const ResolveTicketModal: FC<{ id: string }> = ({ id }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(false);
  const utils = api.useContext();

  const resolveTicket = api.ticket.resolve.useMutation({
    onMutate: () => {
      setLoading(true);
    },
    onError: () => {
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-red-700">
                      We could not resolve this ticket right now, please try
                      again!
                    </p>
                  </div>
                </div>
